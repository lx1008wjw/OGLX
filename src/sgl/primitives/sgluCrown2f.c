...
void sgl_draw_filled_crown(const sgl_crown * par_s_crown)
{
    ...
    SGLbool loc_b_draw_line_loop = SGL_TRUE;
    ...
    if (loc_b_polygon_smooth_backup != SGL_TRUE) {
        loc_b_draw_line_loop = SGL_FALSE;
    }
    else {
        /* Nothing to do */
    }
    ...
    if (loc_b_draw_line_loop && (!(glob_pr_sglStatemachine->b_texture_state)) && (!(glob_pr_sglStatemachine->b_enable_gradient))) {
...
/*+ FUNCTION DESCRIPTION ----------------------------------------------
  ...
  PARAMETERS:
    par_f_thickness -> width of the crown
...
---------------------------------------------------------------------+*/
SGLbool sglu_crown_2f_detect_error(SGLfloat par_f_thickness, SGLfloat par_f_radius, SGLbyte par_b_crown_value, SGLbool par_b_clockwise)
{
    ...
    if (par_f_thickness > par_f_radius) {
        loc_b_error_detected = SGL_TRUE;
    }
    else {
        /* Nothing to do */
    ...
    /* Check width is not nul */
    if (sgluFloatIsEqual(par_f_thickness, 0.0F)) {
        loc_b_error_detected = SGL_TRUE;
    }
    else {
        /* Nothing to do */
                   ...
                   SGLfloat par_f_thickness, SGLbool par_b_clockwise, SGLfloat par_f_angle_in, SGLbyte par_b_crown_value)
{
    ...
    loc_f_angle = mth_acos_degree(loc_f_tmpx, par_f_radius - par_f_thickness);
    ...
    loc_s_sgl_crown.f_radiusin = par_f_radius - par_f_thickness;
    ...
    mth_cos_sin_degree(loc_f_angle, &loc_f_cos, &loc_f_sin);
    loc_s_sgl_crown.f_startoutx = par_f_startinx + (par_f_thickness * loc_f_cos);
    loc_s_sgl_crown.f_startouty = par_f_startiny + (par_f_thickness * loc_f_sin);
...
/*+ FUNCTION DESCRIPTION ----------------------------------------------
  ...
  RETURN:
    None
---------------------------------------------------------------------+*/
void sgluCrown2f(SGLfloat par_f_centerx, SGLfloat par_f_centery, SGLfloat par_f_radius, SGLfloat par_f_thickness,
                 SGLfloat par_f_start_angle, SGLfloat par_f_end_angle, SGLbool par_b_clockwise, SGLbyte par_b_crown_value)
{
    ...
    SGLfloat loc_f_par_thickness = par_f_thickness;

    if (loc_f_par_radius < 0.0F) {
        loc_f_par_radius = 0.0F;
    }

    if (loc_f_par_thickness < 0.0F) {
        loc_f_par_thickness = 0.0F;
    }

    loc_b_error_detected = sglu_crown_2f_detect_error(loc_f_par_thickness, loc_f_par_radius, par_b_crown_value, par_b_clockwise);

    if (!loc_b_error_detected) {
        /* If width is equal to radius, an arc can be drawn */
        if (sgluFloatIsEqual(loc_f_par_thickness, loc_f_par_radius)) {
            if (par_b_crown_value == SGL_CROWN_FILLED) {
                sgluArcSector2f(par_f_centerx, par_f_centery, loc_f_par_radius, par_f_start_angle, par_f_end_angle, par_b_clockwise, SGL_ARC_FILLED);
            }
            else {
                sgluArcSector2f(par_f_centerx, par_f_centery, loc_f_par_radius, par_f_start_angle, par_f_end_angle, par_b_clockwise, SGL_ARC_OUTLINE);
            }
        }
        else {
            SGLfloat loc_f_angle;
            ...
            SGLfloat loc_f_startin_x = (par_f_centerx + (SGLfloat_div(loc_f_vector_x * (loc_f_par_radius - loc_f_par_thickness), loc_f_par_radius)));
            SGLfloat loc_f_startin_y = (par_f_centery + (SGLfloat_div(loc_f_vector_y * (loc_f_par_radius - loc_f_par_thickness), loc_f_par_radius)));
            ...
            if (loc_f_radius <= MTH_RADIUS_TOO_SMALL_MAX) {
                ...
                if (!(sgluFloatIsEqual(loc_f_angle, 0.0F))) {
                    ...
                    sglu_crown_2f(par_f_centerx, par_f_centery, loc_f_par_radius, loc_f_startin_x, loc_f_startin_y, loc_f_par_thickness, par_b_clockwise,
                                  loc_f_angle, par_b_crown_value);
