...
/*+ DEFINITION OF CONSTANTS -------------------------------------------------------
 DESCRIPTION       : Definition of constants of sgl_line.c
 LLR: [OGLX-DDD-sgl_line_constants]
---------------------------------------------------------------------------- +*/
...
void sgl_draw_arrays_gl_lines(void)
{
    ...
    oglxDrawVertexArray((SGLulong) GL_LINES, glob_pr_sglStatemachine->tr_geometry.l_size_array_vertices, (const SGLfloat *) loc_p_vertex_array);
#else
    oglxDrawVertexArray_es2((SGLulong) GL_LINES, glob_pr_sglStatemachine->tr_geometry.l_size_array_vertices, (const SGLfloat *) loc_p_vertex_array);
#endif
...
void sgl_draw_raw_lines(void)
{
    ...
    for (loc_l_i = 0L; loc_l_i < (glob_pr_sglStatemachine->tr_geometry.l_size - 1); loc_l_i += loc_l_inc) {
...
#if !defined(ES2_DEV_ENV) && !defined(SC2_DEV_ENV)
        oglxDrawVertexArray((SGLulong) GL_LINE_LOOP, glob_pr_sglStatemachine->tr_geometry.l_size, (const SGLfloat *) loc_p_vertex_array);
#else
        oglxDrawVertexArray_es2((SGLulong) GL_LINE_LOOP, glob_pr_sglStatemachine->tr_geometry.l_size, (const SGLfloat *) loc_p_vertex_array);
#endif
    }
    else {
        loc_p_vertex_array = glob_pr_sglStatemachine->tr_geometry.p_vertices;
#if !defined(ES2_DEV_ENV) && !defined(SC2_DEV_ENV)
        oglxDrawVertexArray((SGLulong) GL_LINE_STRIP, glob_pr_sglStatemachine->tr_geometry.l_size, (const SGLfloat *) loc_p_vertex_array);
#else
        oglxDrawVertexArray_es2((SGLulong) GL_LINE_STRIP, glob_pr_sglStatemachine->tr_geometry.l_size, (const SGLfloat *) loc_p_vertex_array);
#endif
    }
                               ...
                               SGLbool par_b_haloed_part, SGLbool par_b_colored_part)
{
    ...
    if (glob_pr_sglStatemachine->b_line_rendering_mode == SGL_RAW_OPENGL_LINES) {
        ...
        if (par_b_colored_part) {
            if (glob_pr_sglStatemachine->b_halo_level_state && (glob_pr_sglStatemachine->b_haloing_state != (SGLbyte) SGL_FALSE)) {
...
/*+ FUNCTION DESCRIPTION ----------------------------------------------
  NAME: sgl_draw_continuous_loop_strip
  DESCRIPTION:
    Function shall initiate lines drawing for SGL_LINE_LOOP/STRIP mode
  PARAMETERS:
    par_f_alpha_halo -> width of haloing
    par_f_half_pixel_width_haloed -> half of the width of haloing
    par_f_half_pixel_width -> half of pixel width
  ...
  RETURN:
    None
---------------------------------------------------------------------+*/
void sgl_draw_continuous_loop_strip(SGLfloat par_f_alpha_halo, SGLfloat par_f_half_pixel_width_haloed, SGLfloat par_f_half_pixel_width,
                                    SGLbool par_b_haloed_part, SGLbool par_b_colored_part)
{
    SGLlong loc_l_index_line_width = glob_pr_sglStatemachine->l_linewidth_index;
    SGLfloat loc_f_line_width;
...
void sgl_draw_line(void)
{
    ...
    SGLbool loc_b_haloed_part;
    SGLbool loc_b_colored_part;
    SGLlong loc_l_index_line_width = glob_pr_sglStatemachine->l_linewidth_index;

    switch (glob_pr_sglStatemachine->b_haloing_state) {
    case ((SGLbyte) SGL_TRUE):
    ...
    if (glob_pr_sglStatemachine->tr_geometry.b_type == SGL_LINES) {
        ...
        sgl_draw_continuous_loop_strip(loc_f_alpha_halo, loc_f_half_pixel_width_haloed, loc_f_half_pixel_width, loc_b_haloed_part, loc_b_colored_part);
    }
    return;
}

/*+ FUNCTION DESCRIPTION ----------------------------------------------
  NAME: sgl_backup_for_polygon_smooth
  DESCRIPTION:
    Function shall store OGLX drawing attributes before applying polygon smooth.
  PARAMETERS:
    par_p_backup -> Pointer to store drawing attributes
  RETURN:
    None
---------------------------------------------------------------------+*/
void sgl_backup_for_polygon_smooth(sgl_polygon_smooth_backup * par_p_backup)
{
    ...
    if ((glob_pr_sglStatemachine->b_draw_ellipse) || (glob_pr_sglStatemachine->b_draw_circle) || (glob_pr_sglStatemachine->b_draw_crown)) {
        ...
        glob_pr_sglStatemachine->l_linewidth_index = 1L;
    }
    else {
        /* For shapes, Line width index 0 is enough and allows having straight edges */
        glob_pr_sglStatemachine->l_linewidth_index = 0L;
    }
}

/*+ FUNCTION DESCRIPTION ----------------------------------------------
  NAME: sgl_restore_after_polygon_smooth
  DESCRIPTION:
    Function shall restore OGLX drawing attributes after applying polygon smooth.
  PARAMETERS:
    par_p_backup -> Pointer to restore drawing attributes
  RETURN:
    None
---------------------------------------------------------------------+*/
