/** FILE DESCRIPTION -------------------------------------------------------
 ...
 DESCRIPTION       : sglVertex2f command shall store a vertex
...
{
    if ((glob_pr_sglStatemachine->ul_currentAction == SGL_TRIANGLE_STRIP) || (glob_pr_sglStatemachine->ul_currentAction == SGL_POLYGON)
...
SGLulong sgl_vertex_2f_line_stipple(SGLfloat par_f_x1, SGLfloat par_f_y1, SGLfloat par_f_x2, SGLfloat par_f_y2, SGLulong par_ul_index)
{
    ...
    SGLulong loc_ul_index = par_ul_index;
    ...
    loc_f_current_length = (SGLfloat) glob_pr_sglStatemachine->pr_stipple->tb_length[loc_ul_index] - glob_pr_sglStatemachine->f_last_stipple_length;
    ...
    while ((loc_f_current_length <= loc_f_norm_segment) && loc_b_continue) {
        if (glob_pr_sglStatemachine->pr_stipple->tb_on_off[loc_ul_index] == 1U) {
        ...
        loc_ul_index++;
        if (loc_ul_index >= (SGLulong) loc_b_nb_pattern) {
            loc_ul_index = 0UL;
        }
        else {
            /* Nothing to do */
        ...
        loc_f_current_length += (SGLfloat) glob_pr_sglStatemachine->pr_stipple->tb_length[loc_ul_index];
    ...
    glob_pr_sglStatemachine->f_last_stipple_length =
        (SGLfloat) glob_pr_sglStatemachine->pr_stipple->tb_length[loc_ul_index] - (loc_f_current_length - loc_f_norm_segment);
    ...
    if (loc_l_texture_index < (SGLlong) glob_pr_sglStatemachine->ul_number_of_textures) {
        ...
        if (sgluFloatIsEqual(glob_pr_sglStatemachine->f_current_bbox_width, 0.0F)) {
            ...
            loc_f_u = (glob_pr_sglStatemachine->f_current_dx + (glob_pr_sglStatemachine->f_current_nx
                                                                * SGLfloat_div(par_f_x - glob_pr_sglStatemachine->f_current_xmin,
                                                                               glob_pr_sglStatemachine->f_current_bbox_width))) * loc_f_u_factor;
        }
        ...
        if (sgluFloatIsEqual(glob_pr_sglStatemachine->f_current_bbox_height, 0.0F)) {
            ...
            loc_f_v = (glob_pr_sglStatemachine->f_current_dy + (glob_pr_sglStatemachine->f_current_ny
                                                                * SGLfloat_div(par_f_y - glob_pr_sglStatemachine->f_current_ymin,
                                                                               glob_pr_sglStatemachine->f_current_bbox_height))) * loc_f_v_factor;
        }
...
/*+ FUNCTION DESCRIPTION ----------------------------------------------
  NAME: oglxSetGradientCoordinates
  DESCRIPTION:
    Function shall set texture coordinates for a gradient.
  PARAMETERS:
    par_f_x -> x-coordinate of the vertex
    par_f_y -> y-coordinate of the vertex
  RETURN:
    None
---------------------------------------------------------------------+*/
void oglxSetGradientCoordinates(SGLfloat par_f_x, SGLfloat par_f_y)
{
    SGLlong loc_l_texture_index = glob_pr_sglStatemachine->l_gradient_index;

    /* Robustness case when sglBindTexture has never been called */
    if (loc_l_texture_index < (SGLlong) glob_pr_sglStatemachine->ul_number_of_gradients) {
        SGLfloat loc_f_u_factor = 1.0F;
        SGLfloat loc_f_v_factor = 1.0F;
        SGLfloat loc_f_u;
        SGLfloat loc_f_v;
        if (glob_pr_sglStatemachine->p_gradient_attrib != SGL_NULL) {
            loc_f_u_factor =
                SGLfloat_div((SGLfloat) (glob_pr_sglStatemachine->p_gradient_attrib[loc_l_texture_index].ul_textures_dimension[0]),
                             (SGLfloat) (glob_pr_sglStatemachine->p_gradient_attrib[loc_l_texture_index].ul_dimension_power_2[0]));
            loc_f_v_factor =
                SGLfloat_div((SGLfloat) (glob_pr_sglStatemachine->p_gradient_attrib[loc_l_texture_index].ul_textures_dimension[1]),
                             (SGLfloat) (glob_pr_sglStatemachine->p_gradient_attrib[loc_l_texture_index].ul_dimension_power_2[1]));
        }
        if (sgluFloatIsEqual(glob_pr_sglStatemachine->f_current_bbox_width, 0.0F)) {
            loc_f_u = loc_f_u_factor;
        }
...
void oglxVertex2f(SGLfloat par_f_x, SGLfloat par_f_y)
{
    ...
    if (glob_pr_sglStatemachine->b_texture_state) {
        ...
        if (glob_pr_sglStatemachine->b_enable_gradient) {
            oglxSetGradientCoordinates(par_f_x, par_f_y);
        }
        else {
            /* Nothing to do */
        }
