/** FILE DESCRIPTION -------------------------------------------------------
 ...
 DESCRIPTION       : sglCompressedTexImage2D command shall load a texture 
...
/*+ FUNCTION DESCRIPTION ----------------------------------------------
  ...
  RETURN:	 
    SGLbool -> SGL_TRUE if uncompress is well performed else SGL_FALSE  
---------------------------------------------------------------------+*/
SGLbool oglxUncompressBitmap(SGLbyte par_b_format, SGLlong par_l_width, SGLlong par_l_height, SGLlong par_l_pixel_array_size, const SGLulong * par_pul_pixel)
{
    SGLbool loc_ub_result;
...
#else
    glTexStorage2D(GL_TEXTURE_2D, 1, loc_e_intern_format, (GLsizei) par_l_width, (GLsizei) par_l_height);
    glTexSubImage2D(GL_TEXTURE_2D, (GLint) 0, 0, 0, (GLsizei) par_l_width, (GLsizei) par_l_height, loc_e_format,
...
---------------------------------------------------------------------+*/
SGLbool sgl_comp_teximage2d_detect_error(SGLlong par_l_texture_number, SGLbyte par_b_format, SGLlong par_l_width, SGLlong par_l_height,
...
/*+ FUNCTION DESCRIPTION ----------------------------------------------
  ...
  DESCRIPTION:
    Function shall load a texture from a compressed format.
  PARAMETERS:
    par_l_texture_number -> texture number
    par_b_format -> texture format(SGL_BITMAP_RGB_TRANSPARENT or SGL_BITMAP_RGB_NOT_TRANSPARENT or SGL_BITMAP_ALPHA)
    par_l_width -> texture width, range: [1,glob_pr_sglStatemachine->ul_max_texture_width]
...
void sglCompressedTexImage2D(SGLlong par_l_texture_number, SGLbyte par_b_format, SGLlong par_l_width, SGLlong par_l_height, SGLlong par_l_pixel_array_size,
                             const SGLulong * par_pul_pixel, SGLbyte par_b_texture_application_mode)
{
    SGLbool loc_ub_uncompressed_buffer;

    SGLbool loc_b_error_detected = sgl_comp_teximage2d_detect_error(par_l_texture_number, par_b_format, par_l_width, par_l_height, par_pul_pixel,
