...
SGLbool sglReset_es2(void)
{
    ...
    SGLbool loc_us_return = SGL_TRUE;
    ...
    glShaderSource((GLuint) (glob_pr_sglStatemachine->ui_frag_shader), 1, (const char **) &glob_s_fragment_shader, SGL_NULL);

    /* Compile the source code */
    glCompileShader((GLuint) (glob_pr_sglStatemachine->ui_frag_shader));

    /* Check if compilation succeeded */
    glGetShaderiv((GLuint) (glob_pr_sglStatemachine->ui_frag_shader), GL_COMPILE_STATUS, &loc_b_shader_compiled);

    if (loc_b_shader_compiled == 0) {
    ...
    if (loc_us_return) {
        ...
        glShaderSource((GLuint) (glob_pr_sglStatemachine->ui_vert_shader), 1, (const char **) &glob_s_vertex_shader, SGL_NULL);
        glCompileShader((GLuint) (glob_pr_sglStatemachine->ui_vert_shader));
        glGetShaderiv((GLuint) (glob_pr_sglStatemachine->ui_vert_shader), GL_COMPILE_STATUS, &loc_b_shader_compiled);
        if (loc_b_shader_compiled == 0) {
            loc_us_return = SGL_FALSE;
        }
        else {
        ...
        if (loc_us_return) {
            ...
            glAttachShader((GLuint) (glob_pr_sglStatemachine->ui_program_object), (GLuint) (glob_pr_sglStatemachine->ui_frag_shader));
            glAttachShader((GLuint) (glob_pr_sglStatemachine->ui_program_object), (GLuint) (glob_pr_sglStatemachine->ui_vert_shader));

            /* Bind the custom vertex attribute "pf_vertex_array" to location VERTEX_ARRAY */
            glBindAttribLocation((GLuint) (glob_pr_sglStatemachine->ui_program_object), (GLuint) (glob_pr_sglStatemachine->l_vertex_array_index),
                                 "pf_vertex_array");
            glBindAttribLocation((GLuint) (glob_pr_sglStatemachine->ui_program_object), (GLuint) (glob_pr_sglStatemachine->l_texture_array_index),
                                 "pf_tex_coord");

            /* Link the program */
            glLinkProgram((GLuint) (glob_pr_sglStatemachine->ui_program_object));

            /* Check if linking succeeded in the same way we checked for compilation success */

            glGetProgramiv((GLuint) (glob_pr_sglStatemachine->ui_program_object), GL_LINK_STATUS, &loc_b_linked);
            ...
            if (loc_us_return) {
                ...
                glUseProgram((GLuint) (glob_pr_sglStatemachine->ui_program_object));

                glob_pr_sglStatemachine->i_matrix_location =
                    glGetUniformLocation((GLuint) (glob_pr_sglStatemachine->ui_program_object), "pf_transformation_matrix");
                glUniformMatrix4fv((GLint) (glob_pr_sglStatemachine->i_matrix_location), 1, GL_FALSE, glob_pr_sglStatemachine->f_modelview_matrix);

                glob_pr_sglStatemachine->i_tex_on_location =
                    glGetUniformLocation((GLuint) (glob_pr_sglStatemachine->ui_program_object), "glob_i_texture_active");
                glob_pr_sglStatemachine->i_tex_on_state = -1;
                glUniform1i(glGetUniformLocation((GLuint) (glob_pr_sglStatemachine->ui_program_object), "sampler2d"), 0);

                glob_pr_sglStatemachine->i_clipPlane_uniform[0] = glGetUniformLocation((GLuint) (glob_pr_sglStatemachine->ui_program_object), "pf_clipPlane0");
                glob_pr_sglStatemachine->i_clipPlane_active[0] = glGetUniformLocation((GLuint) (glob_pr_sglStatemachine->ui_program_object), "b_clipPlane0");
                glUniform1i((GLint) (glob_pr_sglStatemachine->i_clipPlane_active[0]), 0);

                glob_pr_sglStatemachine->i_clipPlane_uniform[1] = glGetUniformLocation((GLuint) (glob_pr_sglStatemachine->ui_program_object), "pf_clipPlane1");
                glob_pr_sglStatemachine->i_clipPlane_active[1] = glGetUniformLocation((GLuint) (glob_pr_sglStatemachine->ui_program_object), "b_clipPlane1");
                glUniform1i((GLint) (glob_pr_sglStatemachine->i_clipPlane_active[1]), 0);

                glob_pr_sglStatemachine->i_clipPlane_uniform[2] = glGetUniformLocation((GLuint) (glob_pr_sglStatemachine->ui_program_object), "pf_clipPlane2");
                glob_pr_sglStatemachine->i_clipPlane_active[2] = glGetUniformLocation((GLuint) (glob_pr_sglStatemachine->ui_program_object), "b_clipPlane2");
                glUniform1i((GLint) (glob_pr_sglStatemachine->i_clipPlane_active[2]), 0);

                glob_pr_sglStatemachine->i_clipPlane_uniform[3] = glGetUniformLocation((GLuint) (glob_pr_sglStatemachine->ui_program_object), "pf_clipPlane3");
                glob_pr_sglStatemachine->i_clipPlane_active[3] = glGetUniformLocation((GLuint) (glob_pr_sglStatemachine->ui_program_object), "b_clipPlane3");
                glUniform1i((GLint) (glob_pr_sglStatemachine->i_clipPlane_active[3]), 0);

                glob_pr_sglStatemachine->i_clipPlane_uniform[4] = glGetUniformLocation((GLuint) (glob_pr_sglStatemachine->ui_program_object), "pf_clipPlane4");
                glob_pr_sglStatemachine->i_clipPlane_active[4] = glGetUniformLocation((GLuint) (glob_pr_sglStatemachine->ui_program_object), "b_clipPlane4");
                glUniform1i((GLint) (glob_pr_sglStatemachine->i_clipPlane_active[4]), 0);

                glob_pr_sglStatemachine->i_clipPlane_uniform[5] = glGetUniformLocation((GLuint) (glob_pr_sglStatemachine->ui_program_object), "pf_clipPlane5");
                glob_pr_sglStatemachine->i_clipPlane_active[5] = glGetUniformLocation((GLuint) (glob_pr_sglStatemachine->ui_program_object), "b_clipPlane5");
                glUniform1i((GLint) (glob_pr_sglStatemachine->i_clipPlane_active[5]), 0);
                ...
                glob_pr_sglStatemachine->i_color_location = glGetUniformLocation((GLuint) (glob_pr_sglStatemachine->ui_program_object), "v_color");
                glUniform4fv((GLint) (glob_pr_sglStatemachine->i_color_location), 1, glob_pr_sglStatemachine->tf_current_color_es2);
...
#ifdef SC2_DEV_ENV
static SGLbyte shaderData[] = {
#include "oglx_sc2_shader.dat"
};
...
SGLbool sglReset_sc2(void)
{
    ...
    glProgramBinary((GLuint) (glob_pr_sglStatemachine->ui_program_object), (GLenum) (glob_pr_sglStatemachine->ul_binary_format), shaderData, shaderSize);
    glGetProgramiv((GLuint) (glob_pr_sglStatemachine->ui_program_object), GL_LINK_STATUS, &loc_i_status);

    if (loc_i_status != 0) {

        /* Bind the custom vertex attribute "pf_vertex_array" to location VERTEX_ARRAY */
        glob_pr_sglStatemachine->l_vertex_array_index = glGetAttribLocation((GLuint) (glob_pr_sglStatemachine->ui_program_object), "pf_vertex_array");
        glob_pr_sglStatemachine->l_texture_array_index = glGetAttribLocation((GLuint) (glob_pr_sglStatemachine->ui_program_object), "pf_tex_coord");
        ...
        glob_pr_sglStatemachine->b_sc2_shader_created = SGL_TRUE;

        /* Actually use the created program */
        glUseProgram((GLuint) (glob_pr_sglStatemachine->ui_program_object));

        glob_pr_sglStatemachine->i_matrix_location = glGetUniformLocation((GLuint) (glob_pr_sglStatemachine->ui_program_object), "pf_transformation_matrix");
        glUniformMatrix4fv((GLint) (glob_pr_sglStatemachine->i_matrix_location), 1, GL_FALSE, glob_pr_sglStatemachine->f_modelview_matrix);

        glob_pr_sglStatemachine->i_tex_on_location = glGetUniformLocation((GLuint) (glob_pr_sglStatemachine->ui_program_object), "glob_i_texture_active");
        glob_pr_sglStatemachine->i_tex_on_state = -1;
        glUniform1i(glGetUniformLocation((GLuint) (glob_pr_sglStatemachine->ui_program_object), "sampler2d"), 0);
...
void sgl_reset_text_attributes(void)
{
    ...
    glob_pr_sglStatemachine->l_current_font_color_index = SGLuint32_to_int32(glob_pr_sglStatemachine->ul_nb_colors - 1UL);
