...
/*+ DEFINITION OF CONSTANTS -------------------------------------------------------
 DESCRIPTION       : Definition of constants of sgl_utils.c
 LLR: [OGLX-DDD-sgl_utils_constants]
---------------------------------------------------------------------------- +*/
...
/*+ FUNCTION DESCRIPTION ----------------------------------------------
  ...
  RETURN:	 
    SGLfloat -> the resulting division
---------------------------------------------------------------------+*/
SGLfloat sgluFloatDiv(SGLfloat par_f_float_1, SGLfloat par_f_float_2)
{
    SGLbool loc_b_equal_0 = SGL_FALSE;

    if ((par_f_float_2 < SGL_MIN_F) && (par_f_float_2 > -SGL_MIN_F)) {
        loc_b_equal_0 = SGL_TRUE;
    }
    ...
    return par_f_float_1 / par_f_float_2;
}
...
/*+ FUNCTION DESCRIPTION ----------------------------------------------
  ...
  DESCRIPTION:
    This function shall perform the division between the two given values
  PARAMETERS:
    par_d_double_1 -> divided value
    par_d_double_2 -> divisor value
  RETURN:	 
    SGLdouble -> the resulting division
---------------------------------------------------------------------+*/
SGLdouble sgluDoubleDiv(SGLdouble par_d_double_1, SGLdouble par_d_double_2)
{
    SGLbool loc_b_equal_0 = SGL_FALSE;

    if ((par_d_double_2 < SGL_MIN_D) && (par_d_double_2 > -SGL_MIN_D)) {
        loc_b_equal_0 = SGL_TRUE;
    }

    /* Raise an error if divisor is null */
    if (loc_b_equal_0) {
        oglxSetError(SGL_ERROR_SGLU_DOUBLEDIV, 0U);
    }

    return par_d_double_1 / par_d_double_2;
}
...
/*+ FUNCTION DESCRIPTION ----------------------------------------------
  ...
  RETURN:	 
    SGLuint32 -> the output SGLuint32
---------------------------------------------------------------------+*/
SGLuint32 sglInt8ToUInt32(SGLint8 par_i_value)
{
    ...
    if (par_i_value < 0) {
        ...
        loc_l_return = (SGLuint32) par_i_value;
    }
...
{
    SGLint8 loc_i_return;

    if (par_ui_value > SGL_MAX_INT8_AS_UINT8) {
...
SGLint8 sglInt16ToInt8(SGLint16 par_i_value)
{
    ...
    if ((par_i_value > SGL_MAX_INT8) || (par_i_value < ((SGLint16) SGL_MIN_INT8))) {
...
SGLint16 sglInt32ToInt16(SGLint32 par_i_value)
{
    ...
    if ((par_i_value > SGL_MAX_INT16) || (par_i_value < ((SGLint32) SGL_MIN_INT16))) {
...
{
    SGLuint8 loc_ui_return;

    if ((par_i_value < 0) || (par_i_value > ((SGLint16) SGL_MAX_UINT8))) {
        loc_ui_return = ((par_i_value < ((SGLint16) 0)) ? ((SGLuint8) 0) : SGL_MAX_UINT8);
        oglxSetError(SGL_ERROR_SGL_CONVERTTOUINT8, 0UL);
    }
    else {
        loc_ui_return = (SGLuint8) par_i_value;
...
{
    SGLuint16 loc_ui_return;

    if ((par_i_value < 0) || (par_i_value > ((SGLint32) SGL_MAX_UINT16))) {
        loc_ui_return = ((par_i_value < ((SGLint32) 0) ? ((SGLuint16) 0) : SGL_MAX_UINT16));
        oglxSetError(SGL_ERROR_SGL_CONVERTTOUINT16, 0UL);
    }
    else {
        loc_ui_return = (SGLuint16) par_i_value;
...
SGLint8 sglUInt16ToInt8(SGLuint16 par_ui_value)
{
    ...
    if (par_ui_value > SGL_MAX_INT8_AS_UINT16) {
        loc_i_return = SGL_MAX_INT8;
        oglxSetError(SGL_ERROR_SGL_CONVERTTOINT8, 0UL);
    }
    else {
...
SGLint16 sglUInt32ToInt16(SGLuint32 par_ui_value)
{
    ...
    if (par_ui_value > SGL_MAX_INT16_AS_UINT32) {
        loc_i_return = SGL_MAX_INT16;
        oglxSetError(SGL_ERROR_SGL_CONVERTTOINT16, 0UL);
    }
    else {
...
SGLuint8 sglUInt16ToUInt8(SGLuint16 par_ui_value)
{
    ...
    if (par_ui_value > ((SGLuint16) SGL_MAX_UINT8)) {
        loc_ui_return = SGL_MAX_UINT8;
        oglxSetError(SGL_ERROR_SGL_CONVERTTOUINT8, 0UL);
    }
    else {
...
SGLuint16 sglUInt32ToUInt16(SGLuint32 par_ui_value)
{
    ...
    if (par_ui_value > ((SGLuint32) SGL_MAX_UINT16)) {
        loc_ui_return = SGL_MAX_UINT16;
        oglxSetError(SGL_ERROR_SGL_CONVERTTOUINT16, 0UL);
    }
    else {
...
SGLint8 sglInt32ToInt8(SGLint32 par_i_value)
{
    ...
    if ((par_i_value > ((SGLint32) SGL_MAX_INT8)) || (par_i_value < ((SGLint32) SGL_MIN_INT8))) {
        loc_i_return = ((par_i_value > 0) ? SGL_MAX_INT8 : SGL_MIN_INT8);
        oglxSetError(SGL_ERROR_SGL_CONVERTTOINT8, 0UL);
    }
    else {
...
SGLuint8 sglInt32ToUInt8(SGLint32 par_i_value)
{
    ...
    if ((par_i_value < 0) || (par_i_value > ((SGLint32) SGL_MAX_UINT8))) {
        loc_ui_return = ((par_i_value < ((SGLint32) 0)) ? ((SGLuint8) 0) : SGL_MAX_UINT8);
        oglxSetError(SGL_ERROR_SGL_CONVERTTOUINT8, 0UL);
    }
    else {
        loc_ui_return = (SGLuint8) par_i_value;
...
SGLint8 sglUInt32ToInt8(SGLuint32 par_ui_value)
{
    ...
    if (par_ui_value > SGL_MAX_INT8_AS_UINT32) {
        loc_i_return = SGL_MAX_INT8;
        oglxSetError(SGL_ERROR_SGL_CONVERTTOINT8, 0UL);
    }
    else {
...
SGLuint8 sglUInt32ToUInt8(SGLuint32 par_ui_value)
{
    ...
    if (par_ui_value > ((SGLuint32) SGL_MAX_UINT8)) {
        loc_ui_return = SGL_MAX_UINT8;
        oglxSetError(SGL_ERROR_SGL_CONVERTTOUINT8, 0UL);
    }
    else {
...
/*+ FUNCTION DESCRIPTION ----------------------------------------------
  ...
  RETURN:	 
    SGLuint32 -> the output SGLuint32
---------------------------------------------------------------------+*/
SGLuint32 sglInt16ToUInt32(SGLint16 par_i_value)
{
    SGLuint32 loc_l_return = 0UL;
...
SGLint8 sglInt64ToInt8(SGLint64 par_i_value)
{
    ...
    if ((par_i_value > ((SGLint64) SGL_MAX_INT8)) || (par_i_value < ((SGLint64) SGL_MIN_INT8))) {
        loc_i_return = ((par_i_value > 0) ? SGL_MAX_INT8 : SGL_MIN_INT8);
        oglxSetError(SGL_ERROR_SGL_CONVERTTOINT8, 0UL);
    }
    else {
...
SGLint16 sglInt64ToInt16(SGLint64 par_i_value)
{
    ...
    if ((par_i_value > ((SGLint64) SGL_MAX_INT16)) || (par_i_value < ((SGLint64) SGL_MIN_INT16))) {
        loc_i_return = ((par_i_value > 0) ? SGL_MAX_INT16 : SGL_MIN_INT16);
        oglxSetError(SGL_ERROR_SGL_CONVERTTOINT16, 0UL);
    }
    else {
...
SGLuint8 sglInt64ToUInt8(SGLint64 par_i_value)
{
    ...
    if ((par_i_value < 0) || (par_i_value > ((SGLint64) SGL_MAX_UINT8))) {
        loc_ui_return = ((par_i_value < ((SGLint64) 0)) ? ((SGLuint8) 0) : SGL_MAX_UINT8);
        oglxSetError(SGL_ERROR_SGL_CONVERTTOUINT8, 0UL);
    }
    else {
        loc_ui_return = (SGLuint8) par_i_value;
...
SGLint8 sglUInt64ToInt8(SGLuint64 par_ui_value)
{
    ...
    if (par_ui_value > SGL_MAX_INT8_AS_UINT64) {
        loc_i_return = SGL_MAX_INT8;
        oglxSetError(SGL_ERROR_SGL_CONVERTTOINT8, 0UL);
    }
    else {
...
SGLint16 sglUInt64ToInt16(SGLuint64 par_ui_value)
{
    ...
    if (par_ui_value > SGL_MAX_INT16_AS_UINT64) {
        loc_i_return = SGL_MAX_INT16;
        oglxSetError(SGL_ERROR_SGL_CONVERTTOINT16, 0UL);
    }
    else {
...
SGLuint8 sglUInt64ToUInt8(SGLuint64 par_ui_value)
{
    ...
    if (par_ui_value > ((SGLuint64) SGL_MAX_UINT8)) {
        loc_ui_return = SGL_MAX_UINT8;
        oglxSetError(SGL_ERROR_SGL_CONVERTTOUINT8, 0UL);
    }
    else {
...
SGLint32 sglInt64ToInt32(SGLint64 par_l_value)
{
    ...
    if ((par_l_value > SGL_MAX_INT32) || (par_l_value < ((SGLint64) SGL_MIN_INT32))) {
...
/*+ FUNCTION DESCRIPTION ----------------------------------------------
  ...
  RETURN:	 
    SGLint32 -> the output SGLint32
---------------------------------------------------------------------+*/
SGLint32 sglUInt64ToInt32(SGLuint64 par_ul_value)
{
    SGLint32 loc_l_return = 0L;

    if (par_ul_value > SGL_MAX_INT32_AS_UINT64) {
        loc_l_return = SGL_MAX_INT32;
        oglxSetError(SGL_ERROR_SGL_CONVERTTOINT32, 0UL);
    }
    else {
...
SGLuint16 sglInt64ToUInt16(SGLint64 par_i_value)
{
    ...
    if ((par_i_value < 0) || (par_i_value > ((SGLint64) SGL_MAX_UINT16))) {
        loc_ui_return = ((par_i_value < ((SGLint64) 0)) ? ((SGLuint16) 0) : SGL_MAX_UINT16);
        oglxSetError(SGL_ERROR_SGL_CONVERTTOUINT16, 0UL);
    }
    else {
        loc_ui_return = (SGLuint16) par_i_value;
...
SGLuint16 sglUInt64ToUInt16(SGLuint64 par_ui_value)
{
    ...
    if (par_ui_value > ((SGLuint64) SGL_MAX_UINT16)) {
        loc_ui_return = SGL_MAX_UINT16;
        oglxSetError(SGL_ERROR_SGL_CONVERTTOUINT16, 0UL);
    }
    else {
...
SGLuint32 sglInt64ToUInt32(SGLint64 par_l_value)
{
    ...
    if (par_l_value > SGL_MAX_UINT32_AS_INT64) {
...
/*+ FUNCTION DESCRIPTION ----------------------------------------------
  ...
  RETURN:	 
    SGLint32 -> the output SGLint32
---------------------------------------------------------------------+*/
SGLuint32 sglUInt64ToUInt32(SGLuint64 par_ui_value)
{
    SGLuint32 loc_l_return = 0UL;

    if (par_ui_value > SGL_MAX_UINT32) {
        loc_l_return = SGL_MAX_UINT32;
        oglxSetError(SGL_ERROR_SGL_CONVERTTOUINT32, 0UL);
    }
    else {
        loc_l_return = (SGLuint32) par_ui_value;
    }
...
#else
            loc_ui_return = (SGLuint64) sglFloatToUInt32(par_f_value);
#endif
        }
    }
...
#else
            loc_ui_return = (SGLuint64) sglDoubleToUInt32(par_d_value);
#endif
        }
    }
...
#else
        loc_i_return = sglFloatToInt32(par_f_value);
#endif
    }
...
#else
        loc_i_return = sglDoubleToInt32(par_d_value);
#endif
    }
