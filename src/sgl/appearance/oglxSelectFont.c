...
{
    SGLulong loc_ui_texture_number = par_ui_texture_number;
    /* Check if given texture id is in valid range */
    if (loc_ui_texture_number >= GLF_MAX_NUMBER_OF_TEXTURE) {
        loc_ui_texture_number = 0U;
    }
    else {
        /* Nothing to do */
    }
    ...
    /* Bind texture */
    oglxBindTexture((GLuint) (glob_pr_sglStatemachine->tui_texture_id[loc_ui_texture_number]));
