...
void oglxUpdateColor(SGLlong par_l_index)
{
    ...
    if ((loc_l_index < 0L) || (loc_l_index > (SGLlong) (glob_pr_sglStatemachine->ul_nb_colors - 1UL))) {
        loc_l_index = (SGLlong) (glob_pr_sglStatemachine->ul_nb_colors - 1UL);
    }
    else {
        /* Nothing to do */
    }

    loc_sgl_colormap = (glob_pr_sglStatemachine->tr_color)[loc_l_index];
