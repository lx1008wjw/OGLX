...
#else
    glTexStorage2D(GL_TEXTURE_2D, 1, GL_RGBA, (GLsizei) par_l_width, (GLsizei) par_l_height);
    glTexSubImage2D(GL_TEXTURE_2D, (GLint) 0, (GLint) 0, (GLint) 0, (GLsizei) par_l_width, (GLsizei) par_l_height, GL_RGBA,
                               ...
                               SGLlong par_l_width, SGLlong par_l_height, SGLlong par_l_pixel_array_size, const void *par_p_pixel, SGLbool par_b_compressed)
{
    ...
    if ((par_l_width >= 1L) && (par_l_height >= 1L) && (loc_ul_index < SGL_MAX_STATIC_TEXTURE) && (glob_pr_sglStatemachine->tub_texture_buffer != SGL_NULL)
        && (par_l_width <= (SGLlong) glob_pr_sglStatemachine->ul_max_texture_width)
        && (par_l_height <= (SGLlong) glob_pr_sglStatemachine->ul_max_texture_height) && (par_p_pixel != SGL_NULL)) {
        ...
        if (par_b_compressed) {
            ...
            if (loc_b_uncompressed) {
                ...
                oglxSetError(SGL_ERROR_SGL_CREATESTATICBITMAP, glob_pr_sglStatemachine->ul_static_bitmap_number);
            }
        }
        else {
            SGLulong loc_ul_texture_id;
            ...
            oglxEnlargeTextureKeepDimensions(par_l_width, par_l_height, loc_l_width, loc_l_height, 4L);
            loc_ul_texture_id = oglx_load_texture(loc_l_width, loc_l_height);
