/* Code generated by SCADE Display codegen wrapper */
...
/* Pointer storage */
static sdy_pointer_event_t pointer_events[MAX_POINTERS];
static int last_pointer_event = 0;

/* Keyboard storage */
static sdy_keyboard_event_t keyboard_events[MAX_KEYBOARDS];
...
void BHVR_SetPointer(SGLint32 id, SGLfloat x, SGLfloat y,  SGLint32 button, SGLbool pressed, SGLbool released, SGLint32 modifiers){
int index;
  for (index=0; index<MAX_POINTERS; index++) {
    if (index >= last_pointer_event) {
      last_pointer_event++;
      pointer_events[index].id = id;
      pointer_events[index].position[0] = x;
      pointer_events[index].position[1] = y;
      pointer_events[index].button = button;
      pointer_events[index].pressed = pressed;
      pointer_events[index].released = released;
      pointer_events[index].modifiers = modifiers;
      break;
    }
    else if (pointer_events[index].id == id) {
      pointer_events[index].id = id;
      pointer_events[index].position[0] = x;
      pointer_events[index].position[1] = y;
      pointer_events[index].button = button;
...
/* Setter functions for pointer and keyboard */
void BHVR_SetKeyboard(SGLint32 id, SGLint32 keycode, SGLbool pressed, SGLbool released, SGLint32 modifiers){
  int index;
    for (index=0; index<MAX_KEYBOARDS; index++) {
      if (index >= last_keyboard_event) {
        last_keyboard_event++;
        keyboard_events[index].id = id;
        keyboard_events[index].keycode = keycode;
        keyboard_events[index].pressed = pressed;
        keyboard_events[index].released = released;
        keyboard_events[index].modifiers = modifiers;
        break;
      }
      else if (keyboard_events[index].id == id) {
        keyboard_events[index].id = id;
        keyboard_events[index].keycode = keycode;
        keyboard_events[index].pressed = pressed;
        keyboard_events[index].released = released;
